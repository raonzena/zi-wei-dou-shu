...
- 조사 시 원격 ref가 없고 작업 폴더도 비어 있었다. 기존 앱·설치 의존성은 없다.
...
브라우저 E2E 도구는 Playwright를 후보로 둔다. 채택 전까지는 확정 설치 목록에 넣지 않는다. 입력 → 로딩 → 결과 → 상세 → 공유와 권한·오류 사례를 실제 브라우저에서 검증한다.

검증 도구마다 역할을 구분한다. Vitest 통과만으로 Next.js 페이지 렌더링과 서버·브라우저 전체 동작을 검증했다고 보고하지 않는다.
...
설치 버전은 `package.json`에 정확한 버전으로, 전이 의존성은 `pnpm-lock.yaml`에 기록한다. 설치된 조합:
