...
export function createPalaceReading(
  ...
  return {
    ...
    introduction: entries.length
      ? opposite
        ? `명반에서 이 영역에는 주성이 없습니다. 주성이 없다고 이 영역이 비어 있거나 중요하지 않다는 뜻은 아닙니다. 이럴 때는 맞은편 ${opposite.name}궁에 놓인 ${entries.map((entry) => entry.starName).join('·')}의 기본 성향을 참고해 ${context.focus}을 살펴볼 수 있습니다. 다만 맞은편 궁의 뜻과 별의 성향이 이 영역에 그대로 나타난다고 단정하지 않고, 이 궁의 보조성과 주변 궁의 관계도 함께 확인해야 합니다.`
    ...
    scope:
      opposite && entries.length
        ? `주성이 없는 ${palace.name}궁을 이해하기 위해 맞은편 ${opposite.name}궁의 주성 기본 의미를 참고했습니다. 맞은편 궁의 의미를 ${palace.name}궁에 그대로 옮긴 결론은 아닙니다. 이 궁의 보조성·사화·밝기와 주변 궁의 관계를 종합한 풀이는 포함하지 않습니다.`
        : opposite
          ? `주성이 없는 ${palace.name}궁과 맞은편 ${opposite.name}궁의 주성 기본 의미만으로는 풀이를 제공하기 어렵습니다. 이 궁의 보조성과 주변 궁의 관계를 함께 살펴야 합니다.`
          : entries.length > 1
