...
요청은 90초, 최대 출력 16,000토큰으로 제한하고 SDK 자동 재시도를 끈다. AI에는 12궁·표시 범위의 별·생년사화와 궁의 연결 관계를 보낸다. 원본 출생 정보는 보내지 않으며, 아래 보충 자료 범위의 밝기·대한·올해 유년을 함께 전달한다. 대한의 나이·연도 구간으로 출생 연도를 추정할 수 있음을 고지한다. 사용자 제공 12단계 프롬프트로 전달된 자료의 범위 안에서 해석한다. `store: false`를 사용하되 제공자 측 모든 보관을 배제한다고 간주하지 않는다.
