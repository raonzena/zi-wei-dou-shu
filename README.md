...
- 현재 단계: 초기 설정과 계산 엔진 1차 평가 완료. 초기 iztro 사용과 후속 TypeScript 자체 구현을 결정했으며 출생 정보 검증·시간 정규화에서 iztro 명반 계산까지 서버 내부 흐름을 연결했다. 독립 명반의 선별 대조와 입력·명반 미리보기 연결을 진행했다. 추가 검증·상세 결과·해석·공유는 후속 작업이다.
...
기본 개발 주소는 http://localhost:3000 이다. 첫 화면에서 출생 정보를 입력하면 실제 명반 미리보기를 확인할 수 있다. 저장·개인화 해석·상세·공유는 아직 연결하지 않았다.
...
출생 입력 → 서버 검증·계산 → 12궁·주성 미리보기까지 연결했다. 초기 계산은 윤달 늦은 자시 예외까지 iztro를 따른다. 사용자는 추후 해당 처리가 개선되기를 원하며, 변경 시 경계 검증과 정책 버전 갱신을 수행한다.

1. 결과 표시 항목·용어집을 확정하고 간편·상세 명반과 툴팁을 연결한다.
2. 해석 근거·생성 방식, 소유권·보관 정책을 정하고 결과 저장·복원·공유를 구현한다.
