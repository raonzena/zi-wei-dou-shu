...
초기 엔진으로 iztro를 사용하고, 추후 필요한 명반 기능을 TypeScript로 직접 구현하기로 결정했다. 추가 계산 검증과 상세 결과, 개인화 해석, 공유 기능은 후속 작업으로 남아 있다.
...
기본 개발 주소는 http://localhost:3000 이다. 첫 화면에서 출생 정보를 입력하면 실제 명반 미리보기를 확인할 수 있다. 결과 저장, 개인화 해석, 상세 명반과 공유 기능은 아직 연결하지 않았다.
...
출생 입력 → 서버 검증·계산 → 12궁·주성 미리보기까지 연결했다. 초기 계산은 윤달 늦은 자시 예외까지 iztro를 따른다. 사용자는 추후 윤달과 늦은 자시가 겹칠 때의 처리가 개선되기를 원한다. 해당 규칙을 변경할 때는 날짜·시각 경계를 검증하고 계산 정책 버전을 갱신한다.
