...
- 요구사항을 수행할 수 없어 다른 방향으로 진행한 경우 `docs/requirements.md`에 관련 요구사항 ID 또는 절, 기존 요구사항, 수행할 수 없는 이유와 근거, 변경 전후 방향, 영향 범위, 미결정 사항을 기록한다. 상세 ADR을 연결하더라도, 요구사항 문서에 변경 이유와 방향을 직접 기록한다.
...
- 요구사항의 원본은 `docs/requirements.md`, 아키텍처 결정은 `docs/decisions/`에 유지한다. 작업 문서는 요구사항과 ADR을 연결하고, 해당 내용이 이번 작업에 미친 영향을 설명한다. 수행 불가로 요구사항과 달라진 경우 requirements.md에도 이유·변경 방향을 반드시 기록한다.
